...
| 成果物の接続 | **エッジで明示マッピング**（output artifact → input artifact） | UIでポートを選んで接続 |
...
| **Process（工程）** | 設計・実装・評価などの作業単位。1つのエージェント実行に対応。入力/出力ポート、割当エージェント、SKILL、Goal.md を持つ。 |
...
### 6.1 ドメイン関係
```
Workflow 1───* Process 1───* Port(in/out, =Artifact定義)
Workflow 1───* Edge (from: Process.outPort → to: Process.inPort)
...
artifact_port(id, process_id, direction,        -- direction: 'in'|'out'
        artifact_name, artifact_type,           -- type: file|url|text
        spec_json)                              -- 既定値・必須フラグ等

process_skill(process_id, skill_name, skill_source, skill_ref)

edge(id, workflow_id,
     from_process_id, from_port_id,             -- 上流output
     to_process_id, to_port_id)                 -- 下流input（明示マッピング）
...
artifact_value(id, run_id, port_id, artifact_type,
    file_path, url, text_value)                 -- 実体メタ（fileはworkdir内パス）
```
...
### 8.1 ワークフロー設計UI（DFD）
- キャンバス上に**工程ノード**と**成果物ノード**を配置。
- ドラッグ&ドロップで接続（エッジ）を作成。エッジは「上流output → 下流input」を明示的に結ぶ。
- 工程・成果物を随時**追加/削除**可能。
- 各工程ノードは入力/出力ポート（成果物）を持ち、ポート単位で接続。
- レイアウトは `workflow.layout_json` に永続化。
...
- Goal.md 記述欄で、その工程に接続済みの in/out 成果物名を `/` トリガーで候補表示しショートカット入力。
- 例: `{Review後のUSDM PR}の内容を満たした、{詳細設計書} が完成している。`
  - `/` 入力時に接続済み成果物 `{Review後のUSDM PR}` を候補から選択。
- 内部的にはトークン（`{{artifact:<id>}}` 等）として保存し、実行時に実値へ解決。
...
GET    /api/runs/{run_id}/artifacts/{port_id}/download
